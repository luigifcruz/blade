...
int main(int argc, char **argv) {
    ...
    blade_ata_h_initialize(number_of_workers);
    ...
    for (int i = 0; i < number_of_workers; i++) {
        size_t input_byte_size = blade_ata_h_get_input_size() * sizeof(int8_t) * 2;
        input_buffers[i] = (void*)malloc(input_byte_size);
        blade_pin_memory(input_buffers[i], input_byte_size);

        size_t output_byte_size = blade_ata_h_get_output_size() * sizeof(float);
    ...
    for (int i = 0; i < (BLADE_ATA_MODE_H_ACCUMULATE_RATE * 8); i++) {
        ...
        size_t b_id;
        if (blade_ata_h_dequeue_b(&b_id)) {
            printf("Task B %zu finished.\n", b_id);

            if (blade_ata_h_enqueue_h(b_id, output_buffers[y], y)) {
    ...
    double iteration_time = (time_spent / (BLADE_ATA_MODE_H_ACCUMULATE_RATE * 8)) * 1000;
    printf("Average execution per-iteration: %lf ms.\n", iteration_time);

    blade_ata_h_terminate();
