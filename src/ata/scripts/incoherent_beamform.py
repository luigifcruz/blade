...
f = open("./filterbank_incoherent.bin", "wb")
...
for gname in gnames:
    ...
    for i in range(128):
        ...
        #dd = np.abs(data).sum(axis=-1)
        #dd = dd[[0,1,2,3,4,5,6,8,9,10,11]].sum(axis=0).T
        ##dd.astype("float32").tofile(f)
        
        dd = cp.array(data)
        dd = cp.abs(dd).sum(axis=-1)
        dd = dd[[0,1,2,3,4,5,6,8,9,10,11]].sum(axis=0).T
        dd_cpu = cp.asnumpy(dd)
        dd_cpu.astype("float32").tofile(f)
