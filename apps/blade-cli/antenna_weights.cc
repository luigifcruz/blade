...
void gather_antenna_weights_from_bfr5_cal_b(
	const complex_float_t* cal_b, // [NCHAN=slowest, NPOL, NANTS=fastest)]
    ...
    const size_t calb_ant_stride = 1;
    const size_t calb_pol_stride = nants*calb_ant_stride;
    const size_t calb_chan_stride = npol*calb_pol_stride;
    ...
    for(ant_i = 0; ant_i < nants; ant_i++) {
        for(chan_i = 0; chan_i < number_of_channels; chan_i++) {
            for(pol_i = 0; pol_i < npol; pol_i++) {
                ...
                ] = CAST_COMPLEX_DOUBLE(cal_b[
                    (starting_channel + chan_i) * calb_chan_stride +
                    pol_i * calb_pol_stride +
                    ant_i * calb_ant_stride
                ]);
            }
        }
    }
