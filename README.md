...
This is a struct-like object that defines the shape of the data generated by an interferometer array. The NBEAMS defines the number of beams, the NANTS defines the number of antennas, the NCHANS defines the number of channels, the NTIME defines the number of time samples, and the NPOLS defines the number of polarization. The helper function `getSize` returns the number of elements inside such a shape.
